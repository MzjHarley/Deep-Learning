...
def d_loss_fn(generator, discriminator, batch_z, batch_x, is_training):
    ...
    loss = celoss_ones(d_real_logits) + celoss_zeros(d_fake_logits) + 10. * gp
    return loss, gp
...
def gradient_penalty(discriminator, batch_x, fake_image):
    t = tf.random.uniform([batch_x.shape[0], 1, 1, 1])
    t = tf.broadcast_to(t, batch_x.shape)  # [b,64,64,3]
    interplate = t * batch_x + (1 - t) * fake_image  # Multiply corresponding elements
    with tf.GradientTape() as tape:
        tape.watch([interplate])
        d_interplote_logits = discriminator(interplate, training=True)
    grads = tape.gradient(d_interplote_logits, interplate)  # [b,64,64,3]
    grads = tf.reshape(grads, [grads.shape[0], -1])
    gp = tf.norm(grads, axis=1)  # square root of sum of squares of axis=1'all elements,[b]
...
def train():
    ...
    for epoch in range(3000000):
        for i in range(5):
            ...
            with tf.GradientTape() as tape:
                d_loss, gp = d_loss_fn(generator, discriminator, batch_z, batch_x, is_training=True)
        ...
        if epoch % 100 == 0:
            print(epoch, 'd-loss:', np.float(d_loss.numpy()), 'g-loss:', np.float(g_loss.numpy()), 'gp:',
                  np.float(gp.numpy()))
