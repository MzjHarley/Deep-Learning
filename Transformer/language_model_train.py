...
def get_batch(src, i, bptt):
    ...
    src=torch.cat((src,cat),dim=1) #在这里结尾填上<eos>
...
def train(model, optimizer, train_data, epoch, bptt):
    ...
    for batch, i in enumerate(range(0, train_data.size(0) - 1, bptt)):
        ...
        optimizer.zero_grad()
        output = model(data,data,src_seq_mask=None, trg_seq_mask=get_seq_mask(data))
        ...
        if batch % log_interval == 0 and batch > 0:
            ...
            print('| epoch {:3d} | {:5d}/{:5d} batches | '
                ...
                epoch, batch, len(train_data) // bptt, optimizer._optimizer.param_groups[0]['lr'],
...
def evaluate(eval_model, data_source, bptt):
    ...
    with torch.no_grad():
        for i in range(0, data_source(0) - 1, bptt):
            ...
            output = eval_model(data,data,src_seq_mask=None, trg_seq_mask=get_seq_mask(data))
...
def get_model(ntokens):
    ...
    optimizer = ScheduledOptim(
        optimizer=optim.Adam(transformer_model.parameters(), betas=(0.9, 0.98), eps=1e-09),
        lr=2,
        d_model=512,
        n_warm_steps=4000)
...
def main():
    ...
    transformer_model, optimizer = get_model(ntokens)
    best_model = None
